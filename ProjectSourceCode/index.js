...
function getProfilePicURL(req) {
  return req.cookies.profile_picture_url != null
    ? req.cookies.profile_picture_url
    : "/static/images/placeholders/placeholder_profile.png";
}
...
app.post("/register", async (req, res) => {
  ...
  if (!passwordRegex.test(password)) {
    return res.status(400).render("pages/register", {
      ...
      error: true,
      message:
        "Password must be 8-15 characters long, include at least one lowercase letter, one uppercase letter, and one special character.",
      theme: prefersDarkMode(req),
    });
  }
  try {
    ...
    await db.none("INSERT INTO grocery_lists (user_id) VALUES ($1)", [
      user.user_id,
    ]);
...
app.get("/profile/:userId", async (req, res) => {
  try {
    ...
    const user = await db.one(
      "SELECT username, bio, profile_pic_url FROM users WHERE user_id = $1",
      [userId]
    );
    const recipes = await db.any(
      `
...
app.get("/recipes/:recipe_id", async (req, res) => {
  ...
  try {
    ...
    const estimatedCost = prices.reduce((sum, p) => sum + p, 0).toFixed(2);
    // ------------------------------------------------------------
  ...
  try {
    ...
    console.log(
      "Recipe Name: " +
        ...
        ", Logged In: " +
        isLoggedIn(req) +
        ", Message: Recipe posted successfully! Name: " +
        recipeName +
        ", Error: " +
        false
    );
...
app.post("/recipes/:recipe_id/like", async (req, res) => {
  ...
  try {
    ...
    if (existingLike) {
      ...
      await db.none("DELETE FROM likes WHERE recipe_id = $1 AND user_id = $2", [
        recipe_id,
        user_id,
      ]);
    } else {
      // add like
      await db.none(
        "INSERT INTO likes (recipe_id, user_id, created_at) VALUES ($1, $2, NOW())",
        [recipe_id, user_id]
      );
    }
...
app.get("/liked", async (req, res) => {
  ...
  if (recipes.length === 0) {
    res.render("pages/liked", {
      ...
      error: true,
      message:
        "No liked recipes found. Go to the home page to find some recipes!",
    });
  } else {
    res.render("pages/liked", {
      recipes,
    ...
    const ingredients = await Promise.all(
      items.map(async ({ ingredient_text }) => {
        ...
        try {
          ...
          console.error(
            `[LIST] priceFor ERROR for "${ingredient_text}":`,
            err.message
          );
        }
...
app.get("/settings", async (req, res) => {
  try {
    let user = await db.one("SELECT * FROM users WHERE user_id = $1 LIMIT 1", [
      req.session.userId,
    ]);
    ...
    res.status(500).render("pages/settings", {
      ...
      error: true,
      message:
        "Error retrieving user settings. Please reload the page to try again...",
    });
  }
});
...
app.post("/settings/update", async (req, res) => {
  try {
    ...
    await db.none(
      "UPDATE users SET prefers_dark_mode = $1 WHERE user_id = $2",
